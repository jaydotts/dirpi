...
public: 
  ...
  void Display(); // Make an event display for the event
};
...
void Event::WritePulses() {
  ...
  for (int i=0; i<Pwidth.size(); i++) { 
    ...
    fprintf(outF,"%d ",Pch[i]); // 10=channel number
...
void Event::Display() {
  ...
  fclose(outF);
  char command[100];
  sprintf(command,"DiRPiEventDisplay.csh %d %d",runNum,evtNum);
  system(command);
  DumpPulses();
} // Event::Display
...
public: 
  ...
  void Display(int evtnum); // Make an event display for the passed event number
};
...
void Run::ReadTXT(int fnum) {
 ...
 int prevdRunTime = 0;
 string line;
 Event Evt; // Local version of an event that we fill and then save.
 Evt.Clear();
 Evt.runNum = runNum;
 ...
 while (! done) { // Read through file
  ...
  if (line[0] == 'D' && line[1] == 'A') { // DATA line
    string word = line.substr(5,line.length());  // Cut off "DATA "
    sscanf(word.c_str(),"%d %d %d %d",&evt, &iAddr, &C1, &C2);
    if (iAddr == 0) Evt.evtNum = evt;
...
void Run::ReadPulseBlock(int e) { // Read a pulse block
  ...
  inF.read(reinterpret_cast<char *>(&ibyte),sizeof(ibyte));
  int chan = (int)ibyte;
if (debug && e<3) cout << "ReadPulseBlock e="<<e<<" c="<<chan<<"\n";
  if (chan == 48) chan = 0; // Correct or character format if needed
  if (chan == 49) chan = 1; // Correct or character format if needed
  if (chan<0 || chan>1) {
    cerr << "Illegal channel number: "<<chan<< ". Forcing into chan=0.\n";
...
int main(int argc, char *argv[])
{
  ...
  if (strncmp(argv[1],"pulses",13)==0) { // Pulses
  ...
  if (strncmp(argv[1],"display",13)==0) { // Display
    if (argc < 4) {
      cerr << "Must pass event number for display\n";
    }
    else {
      int evtnum;
